...
const supabaseStoreKey = process.env.SUPABASE_STORE_KEY?.trim() || 'grozersstore'
...
app.post('/api/topups', async (request, response) => {
  ...
  try {
    const invoice = await createCryptoInvoice({
      ...
      description: `GrozersStore balance top-up: $${topup.amount}`,
    })
...
if (botToken) {
  ...
  const botText = {
    ru: {
      ...
      welcome: (name) => [
        `Добро пожаловать, ${name}. Это GrozersStore 🚀`,
        '',
